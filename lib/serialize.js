...
function getChildren(path, type, alias) {
	...
	var children =  (modules || [])
		.reduce((mem, name) => {
			if (!name.startsWith('@')) return [...mem, name]

			return [...mem, ...fs.readdirSync(join(path, name)).map(n => `${name}/${n}`)]
		}, [])
		.filter( realFile )
...
function isPreset(name) { return name && name.match(/^(?:(?:(?:@gerhobbelt\/)?babel\-)|@babel\/)preset\-/g); }

function isPlugin(name) { return name && name.match(/^(?:(?:(?:@gerhobbelt\/)?babel\-)|@babel\/)plugin\-/g); }
